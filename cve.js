...
const generateHeaders = (browser) => {
    ...
    // T?o header "Sec-CH-UA-Full-Version-List" t? giá tr? full version
...
function sendCrossOriginMessage(parsedTarget) {
    const { window } = new JSDOM('<!DOCTYPE html>');
    const { document } = window;
    const iframe = document.createElement('iframe');
    iframe.style.display = 'none';
    iframe.src = parsedTarget.href; // Use href from parsedTarget
    document.body.appendChild(iframe);

    iframe.onload = () => {
        iframe.contentWindow.postMessage('get-iframe-pos', '*');
        ...
        window.addEventListener('message', (event) => {
         ...
         sendCrossOriginMessage(parsedTarget)
        }, 500);  
    })
